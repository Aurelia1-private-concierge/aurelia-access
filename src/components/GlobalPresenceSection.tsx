...
const GlobalPresenceSection = () => {
  return <section className="py-24 md:py-32 bg-background relative text-platinum-light">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {locations.map((location, index) => <motion.div key={location.city} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
          ...
          delay: index * 0.08
        }} className={`group relative p-6 text-center transition-all duration-500 ${location.flagship ? 'bg-card/40 border border-primary/20 hover:border-primary/30' : 'bg-card/20 border border-border/10 hover:border-border/20'}`}>
              {location.flagship && <span className="absolute top-2 right-2 text-[8px] uppercase tracking-wider text-primary/50">
                  HQ
                </span>}
              
              <MapPin className={`w-4 h-4 mx-auto mb-3 transition-colors duration-300 ${location.flagship ? 'text-primary/70' : 'text-muted-foreground/50 group-hover:text-primary/50'}`} strokeWidth={1.5} />
              
              <h3 className="text-sm text-foreground mb-0.5" style={{
            fontFamily: "'Cormorant Garamond', serif"
          }}>
                {location.city}
              </h3>
              <p className="text-[10px] text-platinum-light">{location.timezone}</p>
            </motion.div>)}
        </div>
